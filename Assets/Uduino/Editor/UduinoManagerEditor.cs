...
public class ExitPlayModeOnScriptCompile
{
    ...
    private static void OnEditorUpdate()
    {
        ...
        {
            if (UduinoManager.Instance.enabled)
            {
...
#region Detect Plugins
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (defaultPanel)
        {
            ...
            foreach (KeyValuePair<string, bool> presentExtension in Manager.presentExtentionsMap)
            {
                ...
                bool isActiveValue = EditorGUILayout.Toggle(Manager.existingExtensionsMap[presentExtension.Key], isExtensionActive);
                if (isActiveValue != isExtensionActive)
                {
            ...
            // Extension settings
            if (Manager.activeExtentionsMap.ContainsKey("UduinoDevice_AndroidSerial") && Manager.activeExtentionsMap["UduinoDevice_AndroidSerial"] == true)
            {
            ...
            if (Manager.activeExtentionsMap.ContainsKey("UduinoDevice_AndroidBluetoothLE") && Manager.activeExtentionsMap["UduinoDevice_AndroidBluetoothLE"] == true)
            {
    ...
    public void ArduinoEvents()
    {
        ...
        SerializedProperty valueEvents = serializedObject.FindProperty("OnValueReceivedEvent"); // <-- UnityEvent
    ...
    public void AdvancedSettings()
    {
        ...
        Manager.autoReconnect = EditorGUILayout.Toggle("Reconnect when error", Manager.autoReconnect);
        GUILayout.EndHorizontal();
